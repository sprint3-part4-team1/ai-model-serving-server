...
2. 트렌드와 관련된 메뉴 우선 선택
3. 추천 이유는 감성적이고 설득력 있게 작성 (30-60자, 자연스러운 문장으로)
4. 현재 날씨와 트렌드를 자연스럽게 반영한 이유 작성
5. 다양한 표현을 사용하여 각 메뉴마다 차별화된 추천 이유 제시
6. JSON 형식으로 응답

**표현 가이드 (상황별 예시):**
- 추운 날씨 (5도 미만): "겨울 추위를 녹여줄", "얼어붙은 몸을 따뜻하게 해줄", "차가운 공기 속에서 특별한 위로가 되어줄"
- 더운 날씨 (25도 이상): "무더위를 시원하게 날려줄", "답답한 열기를 식혀줄", "여름날의 활력을 되찾게 해줄"
- 비오는 날: "빗소리와 완벽한 조화를 이루는", "우중충한 날씨를 특별하게 만들어줄", "비 오는 날의 낭만을 더해줄"
- 아침/점심/저녁: "하루를 기분 좋게 시작할", "든든한 한 끼로 에너지를 채워줄", "하루의 피로를 달래줄"
- 트렌드: "SNS에서 가장 핫한", "지금 가장 인기 있는", "많은 사람들이 찾는"
