...
2. 트렌드와 관련된 메뉴 우선 선택
3. **추천 이유는 반드시 30-60자의 완전한 문장으로 작성** (감성적이고 구체적으로)
4. 각 메뉴마다 완전히 다른 표현과 구조를 사용하여 차별화
5. 긍정적이고 매력적인 표현 사용 (부정적 느낌 절대 금지)
6. JSON 형식으로 응답

**좋은 예시 (30-60자의 완전한 문장):**
- "겨울 추위를 녹여줄 진한 풍미와 고소한 맛이 입 안 가득 퍼지는 따뜻한 메뉴"
- "연말 파티 분위기를 한층 더 특별하게 만들어줄 프리미엄 고급 요리"
- "크리스마스의 설렘을 더해줄 풍성한 맛과 화려한 비주얼이 돋보이는 특별 메뉴"
- "얼어붙은 몸을 따뜻하게 감싸주는 부드러운 식감과 깊은 맛의 조화"

**나쁜 예시 (절대 사용 금지):**
- "겨울에 어울리는 고소한 맛" ❌ (너무 짧고 반복적)
- "~에 어울리는 ~" ❌ (단순하고 지루한 패턴)
- "소규모 스테이크" ❌ (부정적 느낌, "작다"는 인상)
- "파티용 메뉴" ❌ (너무 짧고 구체성 없음)

**표현 가이드:**
- 추운 날씨: "겨울 추위를 녹여줄", "얼어붙은 몸을 따뜻하게 해줄", "차가운 공기 속에서 특별한 위로가 되어줄"
- 파티/이벤트: "특별한 순간을 더욱 빛나게 해줄", "소중한 사람들과의 시간을 완성할", "기념일을 더욱 의미있게 만들어줄"
- 맛/식감: "풍부한 맛과 부드러운 식감의 조화", "진한 풍미가 입 안 가득 퍼지는", "고소하고 깊은 맛이 일품인"

**응답 형식:**
{{
  "highlights": [
    {{"menu_id": 1, "name": "아메리카노", "reason": "비 오는 겨울날 움츠러든 몸을 따뜻하게 녹여주는 진한 커피 향과 깊은 맛이 일품"}},
    {{"menu_id": 3, "name": "호떡", "reason": "겨울 간식으로 SNS에서 가장 핫한 트렌드 메뉴, 달콤하고 고소한 맛의 완벽한 조화"}}
  ]
}}

**중요: 각 추천 이유는 반드시 30자 이상의 완전한 문장이어야 합니다!**
...
응답:"""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "당신은 메뉴 큐레이션 전문가입니다. 현재 상황에 가장 적합한 메뉴를 선택하고 30-60자의 감성적이고 완전한 문장으로 설득력 있는 추천 이유를 작성합니다. 반복적인 표현은 절대 사용하지 않으며, 각 메뉴마다 독특하고 매력적인 이유를 제시합니다."
                    },
                    {
                        "role": "user",
                        "content": prompt
