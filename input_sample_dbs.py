"""
샘플 데이터 DB 삽입 스크립트
samples/menu_sample_data_v2.json → MySQL DB
"""
...
def insert_menus(cursor, menus):
    ...
    for menu in menus:
        sql = """
        INSERT INTO menus (id, store_id, name, description, created_at, updated_at)
        VALUES (%s, %s, %s, %s, %s, %s)
        ON DUPLICATE KEY UPDATE
...
def insert_menu_items(cursor, menu_items):
    ...
    for item in menu_items:
        ...
        (id, menu_id, name, description, price, is_available, image_url, created_at, updated_at)
...
def insert_item_ingredients(cursor, ingredients):
    ...
    for ing in ingredients:
        ...
        (id, item_id, ingredient_name, quantity_unit, quantity_value, notes)
        VALUES (%s, %s, %s, %s, %s, %s)
        ON DUPLICATE KEY UPDATE
...
def insert_nutrition_estimates(cursor, estimates):
    ...
    (id, item_id, calories, sugar_g, caffeine_mg, protein_g, fat_g, carbs_g, confidence, last_computed_at)
